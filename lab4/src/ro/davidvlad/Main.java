...
public class Main {
    ...
    private static int[][] matrix_a = GenerateRandomMatrix(700,700);
    private static int[][] matrix_b = GenerateRandomMatrix(700,700);
    private static int[][] matrix_c = GenerateRandomMatrix(700,700);
    private static CountDownLatch latch = new CountDownLatch(1400);
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        latch.await();
        if (!executorServiceAB.awaitTermination(1, TimeUnit.MILLISECONDS))
            executorServiceAB.shutdown();
        if (!executorServiceABC.awaitTermination(1, TimeUnit.MILLISECONDS))
        ...
        for (int i = 0; i < matrix_prod_abc.length; i++) {
            for (int j = 0; j < matrix_prod_abc[0].length; j++) {
                if(matrix_prod_abc[i][j]==0)System.out.println("Error");
            }
//            System.out.println();
        }
